...
namespace ExceptionHandling
{
    public static class ThrowingExceptions
    {
        ...
        public static int CheckEvenNumberAndThrowException(int evenNumber)
        {
            ...
            if (evenNumber % 2 != 0)
            {
                throw new ArgumentException("the number must be even.", nameof(evenNumber));
            }
            else
            {
                return evenNumber;
        ...
        public static int CheckCandidateAgeAndThrowException(int candidateAge, bool isCandidateWoman)
        {
            ...
            {
                throw new ArgumentOutOfRangeException($"Candidate age must be between 16 and {maxAge} for a {(isCandidateWoman ? "woman" : "man")}.", nameof(candidateAge));
            }
        ...
        public static string GenerateUserCode(int day, int month, string username)
        {
            ...
            if (day < 1 || day > 31)
            {
                throw new ArgumentOutOfRangeException("Day cann't be less 1 or greater 31", nameof(day));
            }
            else if (month < 1 || month > 12)
            {
                throw new ArgumentOutOfRangeException("Month cann't be less 1 or greater 12", nameof(month));
            }
            else if (username == null || username == "")
            {
                throw new ArgumentNullException("UserName cann't be null", nameof(username));
            }
